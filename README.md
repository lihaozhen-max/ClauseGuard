...
| `docs/架构图.md` | CG-ARCH-001 | 14 张 Mermaid 图 + ASCII 版（架构、时序、状态机、ER、流水线）；**含"给小白"的讲解层**（起步、名词词典、每图一句人话） |
...
> ⚠️ **跑全量测试会清空开发库的业务数据**（会话级夹具 `reset_business_data`，同时清掉附件落盘目录）——
> 这是**故意**的：否则上一轮残留的状态（例如 AP-004 已被重试成 `done`）会让本轮的断言永远不成立。
> 数据随时可重建（拉取待办 + 解析 + 审查），但如果**演示前刚跑过测试**，页面上的任务会回到"待处理"，
> 先跑一遍 `uv run python scripts/demo_closed_loop.py --instance AP-001` 或直接在界面上点"触发解析/触发审查"。
